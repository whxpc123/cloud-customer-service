...
async function request(path,body){
 ...
 const response=await fetch('/internal/knowledge/'+path,{method:'POST',...(multipart?{body}:{headers:{'Content-Type':'application/json'},...(body?{body:JSON.stringify(body)}:{})})});
 if(!response.ok){
  let details;try{details=await response.json();}catch{}
  const inputError=[400,413].includes(response.status)&&['INVALID_INPUT','FILE_TOO_LARGE'].includes(details?.code)?details.message:null;
  throw new Error(inputError||(response.status===404?'知识库未启用，请以 local,knowledge 环境启动。':response.status===400?'输入无效，请检查资料、问题、条数和阈值。':response.status===413?'文件不能超过 5 MB。':response.status===502?'知识库暂不可用，请检查数据库和百炼向量服务后重试。':`请求失败（${response.status}），请检查 IDEA 日志。`));
 }
 return response.json();
}
let importMode='file';
function setImportMode(mode){
 if(busy)return;importMode=mode;
...
$('import-form').addEventListener('submit',async event=>{
 ...
 const sourceName=$('source-name').value.trim();let body,path;
 if(!sourceName||sourceName.length>120){$('import-status').textContent='请填写 1–120 字符的资料名称。';return;}
 if(importMode==='file'){
  const file=$('import-file').files[0];
  if(!file||!file.size||file.size>5*1024*1024||! /\.(txt|md|markdown|pdf)$/i.test(file.name)){
   $('import-status').textContent='请选择非空的 TXT、Markdown 或 PDF，文件不能超过 5 MB。';return;
  }
  body=new FormData();body.append('file',file);body.append('sourceName',sourceName);path='import/file';
 }else{
  const text=$('import-text').value;if(!text.trim()||text.length>50000){$('import-status').textContent='正文不能为空，且不能超过 50000 字符。';return;}
  body={sourceName,text};path='import';
 }
 lock(true);clear();$('import-form').setAttribute('aria-busy','true');$('import-status').textContent='正在解析正文、生成向量并保存，请稍候…';
 try{
  const result=await request(path,body);
  if(!Number.isInteger(result.importedDocuments)||result.importedDocuments<1)throw new Error('导入响应异常，请检查 IDEA 日志。');
  $('import-status').textContent=`已导入「${result.sourceName}」：${result.characters} 字符，${result.importedDocuments} 个知识块。同名旧内容已替换。现在可以在下方搜索。`;
