...
async function send(event) {
  ...
  try {
    const data = await request(`conversations/${encodeURIComponent(conversationId)}/messages`, { method: 'POST', headers: headers(), body: JSON.stringify({ question, expansionMode: $('expansion-mode').value, rerankEnabled: $('rerank-enabled').value === 'true' }) });
